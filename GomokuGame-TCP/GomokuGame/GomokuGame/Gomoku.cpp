...
Gomoku::Gomoku(HWND hWnd){
	...
	if (hBmp) {
		...
		ShowError(hWnd, GetLastError(), _TEXT("加载棋盘图片时:"));
	}
	...
	if (hBmp) {
		...
		ShowError(hWnd, GetLastError(), _TEXT("加载黑棋图片时:"));
	}
	...
	if (hBmp) {
		...
		ShowError(hWnd, GetLastError(), _TEXT("加载白棋图片时:"));
	}
...
bool Gomoku::Move_Later(HWND hWnd){
	...
	if (!iMap[y][x]) {
		if (Client.Socket) {
			if ((iChessCount_Black == iChessCount_White && !Client.Pos) || (iChessCount_Black > iChessCount_White && Client.Pos)) {
				char Content[MAXBYTE] = { 0 };
				sprintf(Content, "%d\n%d\n%d\n%d\n%d\n", FORWARD, 100, Client.Pos ? WHITE : BLACK, x, y);
				send(Client.Socket, Content, strlen(Content), 0);
			}
		}
		else{
			iMap[y][x] = Later;
...
bool Gomoku::Move_Later(HWND hWnd, int iPos, int X, int Y) {
	CHESS Pos = { 0 };
	if (!iMap[Y][X]) {
		Later = iPos;
		iMap[Y][X] = iPos;
		...
		//if (BLACK == Later)iChessCount_Black--;
		//if (WHITE == Later)iChessCount_White--;
		if (BLACK == iPos)iChessCount_Black++;
		if (WHITE == iPos)iChessCount_White++;
		if (iChessCount_Black + iChessCount_White >= MAP_WIDE * MAP_HIGH) {
			_tcscpy(tcResult, _TEXT("平局"));
			MessageBox(hWnd, _TEXT("平局"), _TEXT("提示"), MB_OK);
			return true;
...
bool Gomoku::DeleFile(HWND hWnd, int Index) {
	...
	for (int i = Index + 1; i < iFileCount; i++){
		...
		if (DeleteFile(FileName)) {
			...
			TCHAR Content_[MAXBYTE] = { 0 };
			_stprintf(Content_, _TEXT("删除%s时："), FileName);
			ShowError(hWnd, GetLastError(), Content_);
